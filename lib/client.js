window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      "card.description":"新会话首问截取首句为临时标题，首轮问答结束后异步调用独立模型生成正式标题，过滤 think 标签，每会话仅一次，手动改名后不再覆盖。",
    ...
    const en = {
      ...
      "card.description":"Immediate fallback from first sentence, then async LLM title after first round, think-tag filtered, once per session, respects user rename.",
